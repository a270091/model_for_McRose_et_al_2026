...
"""
    ...
    # now we do three runs, varying how much ligand are we adding
    ...
    # initial condition: Fe' and Fe bound to ligand added = 0, FeEDTA = 100 nM
    InitCon2 = [0.0, 0.0, FeEDTA0]
    sol3 = solve_ivp(model2EntB, tspan, InitCon2, method='BDF', rtol=1.0e-12, atol=1.0e-20, t_eval=teval)
    
    #---------------------------------------------------------------
    # make a plot
    #---------------------------------------------------------------
    fig,ax = plt.subplots()
    ax.semilogy(sol1.t, sol1.y[0], label="Fe' (25 nM Enterobactin)")
    ax.semilogy(sol1.t, sol1.y[1], label='FeEntB')
    ax.semilogy(sol2.t, sol2.y[0], '--', label="Fe' (50 nM Enterobactin)")
    ax.semilogy(sol2.t, sol2.y[1], '--', label='FeEntB')
    ax.semilogy(sol3.t, sol3.y[0], '-.', label="Fe' (100 nM Enterobactin)")
